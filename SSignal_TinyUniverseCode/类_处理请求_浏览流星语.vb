...
Partial Public Class 类_处理请求
    ...
    Private Structure 查看流星语_复合数据
        Dim 文本编号, 评论数量, 点赞数量 As Long
        Dim 英语用户名, 标题, 正文, 讯友标签, 本国语讯宝地址 As String
        Dim 类型 As 流星语类型_常量集合
        Dim 文本库号 As Short
        Dim 访问权限 As 流星语访问权限_常量集合
    End Structure
    ...
    Public Function 获取流星语列表() As String
        ...
        If EnglishUsername.Length > 最大值_常量集合.英语用户名长度 Then Return Nothing
        Dim 第几页 As Long
        ...
        If 跨进程锁.WaitOne = True Then
            Try
                ...
                ReDim 流星语(每页条数 - 1)
跳转点1:
                结果 = 数据库_获取流星语列表(EnglishSSAddress, EnglishUsername, 流星语, 流星语数量, 每页条数, 第几页, 总数, 是作者)
        ...
        If 流星语数量 > 0 Then
            ...
            For I = 0 To 流星语数量 - 1
                ...
                With 流星语(I)
                    ...
                    If 是作者 Then
                        文本写入器.Write("<PERMISSION>" & .访问权限 & "</PERMISSION>")
                        If .访问权限 = 流星语访问权限_常量集合.某标签讯友 Then
                            文本写入器.Write("<TAG>" & 替换XML敏感字符(.讯友标签) & "</TAG>")
                        End If
                    End If
                    Select Case .类型
    ...
    Private Function 数据库_获取流星语列表(ByVal 英语讯宝地址 As String, ByVal 英语用户名 As String, ByRef 流星语() As 流星语_复合数据,
                                 ByRef 流星语数量 As Integer, ByVal 每页条数 As Integer, ByVal 第几页 As Long, ByRef 总数 As Long, ByRef 是作者 As Boolean) As 类_SS包生成器
        Dim 读取器 As 类_读取器_外部 = Nothing
        Try
            Dim 列添加器 As 类_列添加器
            Dim 筛选器 As 类_筛选器
            If 英语讯宝地址.StartsWith(英语用户名 & 讯宝地址标识) = False OrElse 英语讯宝地址.EndsWith(讯宝地址标识 & 域名_英语) = False Then
                列添加器 = New 类_列添加器
                ...
                Dim 指令 As New 类_数据库指令_请求获取数据(主数据库, "讯友录", 筛选器, 1, 列添加器, , "#用户名讯友")
                Dim 找到了 As Boolean
                ...
                If 找到了 = True Then
                    ...
                    列添加器 = New 类_列添加器
                    列添加器.添加列_用于筛选器("英语用户名", 筛选方式_常量集合.等于, 英语用户名)
                    列添加器.添加列_用于筛选器("访问权限", 筛选方式_常量集合.小于, 流星语访问权限_常量集合.某标签讯友)
                    筛选器.添加一组筛选条件(列添加器)
                    If String.IsNullOrEmpty(标签一) = False Then
                        列添加器 = New 类_列添加器
                        列添加器.添加列_用于筛选器("英语用户名", 筛选方式_常量集合.等于, 英语用户名)
                        列添加器.添加列_用于筛选器("访问权限", 筛选方式_常量集合.等于, 流星语访问权限_常量集合.某标签讯友)
                        列添加器.添加列_用于筛选器("讯友标签", 筛选方式_常量集合.等于, 标签一)
                        筛选器.添加一组筛选条件(列添加器)
                    End If
                    If String.IsNullOrEmpty(标签二) = False Then
                        ...
                        列添加器.添加列_用于筛选器("英语用户名", 筛选方式_常量集合.等于, 英语用户名)
                        列添加器.添加列_用于筛选器("访问权限", 筛选方式_常量集合.等于, 流星语访问权限_常量集合.某标签讯友)
                        列添加器.添加列_用于筛选器("讯友标签", 筛选方式_常量集合.等于, 标签二)
                        筛选器.添加一组筛选条件(列添加器)
                    End If
                    列添加器 = New 类_列添加器
                    列添加器.添加列_用于获取数据(New String() {"编号", "类型", "样式", "标题", "文本库号", "文本编号", "评论数量", "点赞数量", "置顶"})
                    指令 = New 类_数据库指令_请求获取数据(主数据库, "流星语", 筛选器, , 列添加器, 100, "#用户名置顶编号")
                ...
                Else
                    列添加器 = New 类_列添加器
                    列添加器.添加列_用于筛选器("英语用户名", 筛选方式_常量集合.等于, 英语用户名)
                    列添加器.添加列_用于筛选器("访问权限", 筛选方式_常量集合.等于, 流星语访问权限_常量集合.任何人)
                    筛选器 = New 类_筛选器
                    筛选器.添加一组筛选条件(列添加器)
                    列添加器 = New 类_列添加器
                    列添加器.添加列_用于获取数据(New String() {"编号", "类型", "样式", "标题", "文本库号", "文本编号", "评论数量", "点赞数量", "置顶"})
                    Dim 指令2 As New 类_数据库指令_请求快速获取分页数据(主数据库, "流星语", "#用户名置顶编号任何人", 筛选器, , 列添加器, 第几页, 每页条数)
            ...
            Else
                ...
                是作者 = True
                列添加器 = New 类_列添加器
                列添加器.添加列_用于获取数据(New String() {"编号", "类型", "样式", "标题", "文本库号", "文本编号", "评论数量", "点赞数量", "置顶", "访问权限", "讯友标签"})
                Dim 指令2 As New 类_数据库指令_请求快速获取分页数据(主数据库, "流星语", "#用户名置顶编号", 筛选器, , 列添加器, 第几页, 每页条数)
                Dim 读取器2 As 类_读取器_快速分页 = 指令2.执行()
                总数 = 读取器2.记录总数量
                While 读取器2.读取
                    With 流星语(流星语数量)
    ...
    Public Function 获取流星语() As String
        ...
        If 跨进程锁.WaitOne = True Then
            Try
                ...
                End If
                流星语 = New 查看流星语_复合数据
                结果 = 数据库_获取流星语(EnglishSSAddress, 流星语编号, 流星语, 不是讯友)
                If 结果.查询结果 <> 查询结果_常量集合.成功 Then
                    Select Case 结果.查询结果
                        Case 查询结果_常量集合.无权操作
                            Return XML无权操作
                ...
                Const 每页条数 As Integer = 20
                ReDim 评论(每页条数 - 1)
                结果 = 数据库_获取更多评论(EnglishSSAddress, 流星语编号, 0, 评论, 评论数量, 每页条数, 不是讯友)
                If 结果.查询结果 <> 查询结果_常量集合.成功 Then
                    Select Case 结果.查询结果
                        Case 查询结果_常量集合.无权操作
                            Return XML无权操作
        ...
        Else
            Return XML失败
        End If
        If String.IsNullOrEmpty(流星语.英语用户名) = False Then
            Dim 变长文本 As New StringBuilder(1000)
            Dim 文本写入器 As New StringWriter(变长文本)
            文本写入器.Write("<SUCCEED>")
            文本写入器.Write("<TYPE>" & 流星语.类型 & "</TYPE>")
            If 流星语.类型 <> 流星语类型_常量集合.转发 Then
                文本写入器.Write("<ENGLISH2>" & 替换XML敏感字符(流星语.英语用户名 & 讯宝地址标识 & 域名_英语) & "</ENGLISH2>")
                ...
                If 不是讯友 = True Then 文本写入器.Write("<NOTSSPAL/>")
                If 评论数量 > 0 Then
                    Dim I As Short
                    For I = 0 To 评论数量 - 1
                        文本写入器.Write("<COMMENT>")
    ...
    Private Function 数据库_获取流星语(ByVal 英语讯宝地址 As String, ByVal 编号 As Long, ByRef 流星语 As 查看流星语_复合数据, ByRef 不是讯友 As Boolean) As 类_SS包生成器
        Dim 读取器 As 类_读取器_外部 = Nothing
        Try
            ...
            筛选器.添加一组筛选条件(列添加器)
            列添加器 = New 类_列添加器
            列添加器.添加列_用于获取数据(New String() {"已删除", "英语用户名", "类型", "标题", "文本库号", "文本编号", "评论数量", "点赞数量", "访问权限", "讯友标签"})
            ...
            If 英语讯宝地址.StartsWith(流星语.英语用户名 & 讯宝地址标识) = False OrElse 英语讯宝地址.EndsWith(讯宝地址标识 & 域名_英语) = False Then
                If 流星语.访问权限 = 流星语访问权限_常量集合.只有我 Then
                    Return New 类_SS包生成器(查询结果_常量集合.无权操作)
                End If
                列添加器 = New 类_列添加器
                列添加器.添加列_用于筛选器("英语用户名", 筛选方式_常量集合.等于, 流星语.英语用户名)
                列添加器.添加列_用于筛选器("英语讯宝地址", 筛选方式_常量集合.等于, 英语讯宝地址)
                筛选器 = New 类_筛选器
                筛选器.添加一组筛选条件(列添加器)
                列添加器 = New 类_列添加器
                ...
                读取器.关闭()
                If 找到了 = True Then
                    If 流星语.访问权限 = 流星语访问权限_常量集合.某标签讯友 Then
                        If String.Compare(流星语.讯友标签, 标签一) <> 0 AndAlso String.Compare(流星语.讯友标签, 标签二) <> 0 Then
                            Return New 类_SS包生成器(查询结果_常量集合.无权操作)
                        End If
                    End If
                Else
                    If 流星语.访问权限 >= 流星语访问权限_常量集合.全部讯友 Then
                        Return New 类_SS包生成器(查询结果_常量集合.无权操作)
                    End If
                    不是讯友 = True
                End If
            End If
            Select Case 流星语.类型
                Case 流星语类型_常量集合.图文, 流星语类型_常量集合.转发
                    列添加器 = New 类_列添加器
            ...
            End Select
            列添加器 = New 类_列添加器
            列添加器.添加列_用于筛选器("英语讯宝地址", 筛选方式_常量集合.等于, 流星语.英语用户名 & 讯宝地址标识 & 域名_英语)
            筛选器 = New 类_筛选器
            筛选器.添加一组筛选条件(列添加器)
            列添加器 = New 类_列添加器
    ...
    Public Function 获取更多评论() As String
        ...
        If 跨进程锁.WaitOne = True Then
            Try
                ...
                Const 每页条数 As Integer = 20
                ReDim 评论(每页条数 - 1)
                结果 = 数据库_获取更多评论(EnglishSSAddress, 流星语编号, 早于, 评论, 评论数量, 每页条数, 不是讯友)
                If 结果.查询结果 <> 查询结果_常量集合.成功 Then
                    Select Case 结果.查询结果
                        Case 查询结果_常量集合.无权操作
                            Return XML无权操作
    ...
    Private Function 数据库_获取更多评论(ByVal 英语讯宝地址 As String, ByVal 流星语编号 As Long, ByVal 早于 As Long,
                                ByRef 评论() As 评论_复合数据, ByRef 评论数量 As Integer, ByVal 每页条数 As Integer, ByRef 不是讯友 As Boolean) As 类_SS包生成器
        Dim 读取器 As 类_读取器_外部 = Nothing
        Try
            Dim 列添加器 As New 类_列添加器
            ...
            筛选器.添加一组筛选条件(列添加器)
            列添加器 = New 类_列添加器
            列添加器.添加列_用于获取数据(New String() {"已删除", "英语用户名", "访问权限", "讯友标签"})
            Dim 指令 As New 类_数据库指令_请求获取数据(主数据库, "流星语", 筛选器, 1, 列添加器, , 主键索引名)
            Dim 已删除 As Boolean
    ...
    Public Function 获取更多回复() As String
        ...
        If 跨进程锁.WaitOne = True Then
            Try
                ...
                Const 每页条数 As Integer = 20
                ReDim 回复(每页条数 - 1)
                结果 = 数据库_获取更多回复(EnglishSSAddress, 流星语编号, 评论编号, 早于, 回复, 回复数量, 每页条数, 不是讯友)
                If 结果.查询结果 <> 查询结果_常量集合.成功 Then
                    Select Case 结果.查询结果
                        Case 查询结果_常量集合.无权操作
                            Return XML无权操作
    ...
    Private Function 数据库_获取更多回复(ByVal 英语讯宝地址 As String, ByVal 流星语编号 As Long, ByVal 评论编号 As Long,
                                ByVal 早于 As Long, ByRef 回复() As 回复_复合数据, ByRef 回复数量 As Integer, ByVal 每页条数 As Integer, ByRef 不是讯友 As Boolean) As 类_SS包生成器
        Dim 读取器 As 类_读取器_外部 = Nothing
        Try
            Dim 列添加器 As New 类_列添加器
            列添加器.添加列_用于筛选器("编号", 筛选方式_常量集合.等于, 流星语编号)
