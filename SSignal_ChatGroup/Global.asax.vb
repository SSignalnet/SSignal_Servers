...
Public Class Global_asax
    ...
    Sub Application_Start(ByVal sender As Object, ByVal e As EventArgs)
        ...
        Catch ex As Exception
            Return
        End Try
        Dim 跨进程锁 As New Mutex(False, "Mu_SSG")
        ...
        If 副数据库 IsNot Nothing Then
            ...
            If 主数据库 IsNot Nothing Then
                Application.Add("Mu_SSG", 跨进程锁)
                Application.Add("Rb_SSG", 主数据库)
                Application.Add("Nb_SSG", 副数据库)
                Dim 启动器 As New 类_启动器(跨进程锁, 副数据库)
                Application.Add("Ln_SSG", 启动器)
                启动器.启动()
            End If
        End If
    End Sub
    ...
    Sub Application_End(ByVal sender As Object, ByVal e As EventArgs)
        ' 应用程序结束时激发
        Dim 主数据库 As 类_数据库 = Application.Get("Rb_SSG")
        If 主数据库 IsNot Nothing Then 主数据库.关闭()
        Dim 副数据库 As 类_数据库 = Application.Get("Nb_SSG")
        If 副数据库 IsNot Nothing Then 副数据库.关闭()
        Dim 跨进程锁 As Mutex = Application.Get("Mu_SSG")
