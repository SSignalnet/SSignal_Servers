...
Partial Public Class 类_处理请求
    ...
    Public Function 发布流星语() As 类_SS包生成器
        ...
        If 跨进程锁.WaitOne = True Then
            Try
                ...
                Dim 结果 As 类_SS包生成器 = 数据库_验证连接凭据(EnglishSSAddress, Credential, GroupID, 角色, 本国语讯宝地址)
                If 结果.查询结果 <> 查询结果_常量集合.凭据有效 Then
                    Return 结果
                End If
                If 角色 < 群角色_常量集合.管理员 Then
                ...
                Select Case 流星语类型
                    Case 流星语类型_常量集合.图文
                        ...
                        文本写入器.Close()
                        SS包生成器.添加_有标签("E", EnglishSSAddress)
                ...
                End Select
                流星语编号 = 0
                结果 = 数据库_保存流星语(GroupID, 流星语类型, 样式, 标题, SS包, 流星语编号)
                If 结果.查询结果 <> 查询结果_常量集合.成功 Then
                    Return 结果
                End If
                Select Case 流星语类型
                    Case 流星语类型_常量集合.图文
                        If SS包解读器2 IsNot Nothing Then
                            Dim 路径 As String = Context.Server.MapPath("/") & "App_Data\MR\" & GroupID
                ...
                End Select
                Return 数据库_保存小宇宙操作记录(EnglishSSAddress, 小宇宙操作代码_常量集合.发布, 流星语编号, 0, 0)
    ...
    Private Function 数据库_获取最近发布次数(ByVal 英语讯宝地址 As String, ByRef 次数 As Integer) As 类_SS包生成器
        ...
        Try
            ...
            列添加器.添加列_用于筛选器("操作代码", 筛选方式_常量集合.等于, 小宇宙操作代码_常量集合.发布)
            Dim 筛选器 As New 类_筛选器
            筛选器.添加一组筛选条件(列添加器)
            Dim 指令 As New 类_数据库指令_请求获取数据(副数据库, "小宇宙操作记录", 筛选器, , 列添加器, 100, "#地址时间")
            读取器 = 指令.执行()
            While 读取器.读取
                次数 += 1
            End While
    ...
    Private Function 数据库_保存流星语(ByVal 群编号 As Long, ByVal 类型 As 流星语类型_常量集合, ByVal 样式 As 流星语列表项样式_常量集合,
                               ByVal 标题 As String, ByVal SS包() As Byte, ByRef 流星语编号 As Long) As 类_SS包生成器
        Dim 读取器 As 类_读取器_外部 = Nothing
        Dim 文本库号 As Short
        Dim 文本编号 As Long
...
跳转点1:
    ...
    Public Function 评论流星语() As String
        ...
        If 跨进程锁.WaitOne = True Then
            Try
                ...
                Dim 本国语讯宝地址 As String = Nothing
                Dim 角色 As 群角色_常量集合 = 群角色_常量集合.无
                结果 = 数据库_验证连接凭据(EnglishSSAddress, Credential, 群编号, 角色, 本国语讯宝地址)
                If 结果.查询结果 <> 查询结果_常量集合.凭据有效 Then
                ...
                End If
                Dim 评论编号 As Long
                结果 = 数据库_评论流星语(流星语编号, EnglishSSAddress, 本国语讯宝地址, Text, 评论编号)
                If 结果.查询结果 <> 查询结果_常量集合.成功 Then
                    Select Case 结果.查询结果
                        Case 查询结果_常量集合.无权操作
                            Return XML无权操作
                ...
                End If
                If String.IsNullOrEmpty(Text) Then
                    结果 = 数据库_保存小宇宙操作记录(EnglishSSAddress, 小宇宙操作代码_常量集合.点赞, 流星语编号, 0, 0)
                Else
                    结果 = 数据库_保存小宇宙操作记录(EnglishSSAddress, 小宇宙操作代码_常量集合.评论, 流星语编号, 0, 0)
                End If
                If 结果.查询结果 <> 查询结果_常量集合.成功 Then
                    If 结果.查询结果 = 查询结果_常量集合.出错 Then
                        Return XML错误信息(结果.出错提示文本)
    ...
    Private Function 数据库_获取所属的群(ByVal 流星语编号 As Long, ByRef 群编号 As Long) As 类_SS包生成器
        Dim 读取器 As 类_读取器_外部 = Nothing
        Try
            Dim 列添加器 As New 类_列添加器
    ...
    Private Function 数据库_是否已点赞(ByVal 英语讯宝地址 As String, ByVal 流星语编号 As Long, ByVal 评论编号 As Long,
        ...
        Try
            ...
            列添加器.添加列_用于筛选器("操作代码", 筛选方式_常量集合.等于, 小宇宙操作代码_常量集合.点赞)
            Dim 筛选器 As New 类_筛选器
            筛选器.添加一组筛选条件(列添加器)
            Dim 指令 As New 类_数据库指令_请求获取数据(副数据库, "小宇宙操作记录", 筛选器, 1, 列添加器, , "#地址操作")
            读取器 = 指令.执行()
            While 读取器.读取
                已点赞 = True
                Exit While
    ...
    Private Function 数据库_获取最近评论或回复次数(ByVal 英语讯宝地址 As String, ByRef 次数 As Integer) As 类_SS包生成器
        ...
        Try
            Dim 列添加器 As New 类_列添加器
            列添加器.添加列_用于筛选器("英语讯宝地址", 筛选方式_常量集合.等于, 英语讯宝地址)
            列添加器.添加列_用于筛选器("操作代码", 筛选方式_常量集合.不等于, 小宇宙操作代码_常量集合.发布)
            Dim 筛选器 As New 类_筛选器
            筛选器.添加一组筛选条件(列添加器)
            Dim 指令 As New 类_数据库指令_请求获取数据(副数据库, "小宇宙操作记录", 筛选器, , 列添加器, 100, "#地址时间")
            读取器 = 指令.执行()
    ...
    Private Function 数据库_评论流星语(ByVal 流星语编号 As Long, ByVal 英语讯宝地址 As String,
                               ByVal 本国语讯宝地址 As String, ByVal 文本 As String, ByRef 评论编号 As Long) As 类_SS包生成器
        Dim 读取器 As 类_读取器_外部 = Nothing
        Dim 文本库号 As Short
        Dim 文本编号 As Long
        Try
...
跳转点1:
    ...
    Public Function 回复评论() As String
        ...
        If 跨进程锁.WaitOne = True Then
            Try
                ...
                结果 = 数据库_回复评论(流星语编号, 评论编号, 回复对象编号, EnglishSSAddress, 本国语讯宝地址, Text, 回复编号, 回复)
                If 结果.查询结果 <> 查询结果_常量集合.成功 Then
                    Select Case 结果.查询结果
                        Case 查询结果_常量集合.无权操作
                            Return XML无权操作
                ...
                End If
                If String.IsNullOrEmpty(Text) Then
                    结果 = 数据库_保存小宇宙操作记录(EnglishSSAddress, 小宇宙操作代码_常量集合.点赞, 流星语编号, 评论编号, 回复对象编号)
                Else
                    结果 = 数据库_保存小宇宙操作记录(EnglishSSAddress, 小宇宙操作代码_常量集合.回复, 流星语编号, 评论编号, 回复对象编号)
                End If
                If 结果.查询结果 <> 查询结果_常量集合.成功 Then
                    If 结果.查询结果 = 查询结果_常量集合.出错 Then
                        Return XML错误信息(结果.出错提示文本)
    ...
    Private Function 数据库_回复评论(ByVal 流星语编号 As Long, ByVal 评论编号 As Long, ByVal 回复对象编号 As Long,
                              ByVal 英语讯宝地址 As String, ByVal 本国语讯宝地址 As String, ByVal 文本 As String, ByRef 回复编号 As Long, ByRef 回复 As 回复_常量集合) As 类_SS包生成器
        Dim 读取器 As 类_读取器_外部 = Nothing
        Dim 文本库号 As Short
        Dim 文本编号 As Long
        Try
...
跳转点1:
    ...
    Private Function 数据库_设为置顶(ByVal 群编号 As Long, ByVal 流星语编号 As Long) As 类_SS包生成器
        Dim 读取器 As 类_读取器_外部 = Nothing
        Try
            Dim 列添加器 As New 类_列添加器
            ...
            列添加器 = New 类_列添加器
            列添加器.添加列_用于获取数据("编号")
            Dim 指令 As New 类_数据库指令_请求获取数据(主数据库, "流星语", 筛选器, , 列添加器, 10, "#群编号置顶编号")
            Dim 编号(2) As Long
            Dim 编号数量 As Integer
            读取器 = 指令.执行()
            While 读取器.读取
    ...
    Private Function 数据库_取消置顶(ByVal 群编号 As Long, ByVal 流星语编号 As Long) As 类_SS包生成器
        Dim 读取器 As 类_读取器_外部 = Nothing
        Try
            Dim 列添加器 As New 类_列添加器
            列添加器.添加列_用于筛选器("群编号", 筛选方式_常量集合.等于, 群编号)
    ...
    Public Function 删除流星语() As String
        ...
        Else
            Return XML失败
        End If
        Dim 路径 As String = Context.Server.MapPath("/") & "App_Data\MR\" & 群编号 & "\" & 流星语编号
        Select Case 类型
            Case 流星语类型_常量集合.图文
                Dim 路径2 As String
                Dim I As Integer
    ...
    Private Function 数据库_删除流星语(ByVal 流星语编号 As Long, ByRef 类型 As 流星语类型_常量集合, ByRef 样式 As 流星语列表项样式_常量集合) As 类_SS包生成器
        Dim 读取器 As 类_读取器_外部 = Nothing
        Try
            Dim 列添加器 As New 类_列添加器
            列添加器.添加列_用于筛选器("编号", 筛选方式_常量集合.等于, 流星语编号)
        ...
        If 跨进程锁.WaitOne = True Then
            Try
                ...
                If 角色 < 群角色_常量集合.成员_不可发言 Then
                    Return XML无权操作
                End If
                结果 = 数据库_删除评论(EnglishSSAddress, 角色, 流星语编号, 评论编号)
                If 结果.查询结果 <> 查询结果_常量集合.成功 Then
                    If 结果.查询结果 = 查询结果_常量集合.出错 Then
                        Return XML错误信息(结果.出错提示文本)
                    Else
    ...
    Private Function 数据库_删除评论(ByVal 英语讯宝地址 As String, ByVal 角色 As 群角色_常量集合, ByVal 流星语编号 As Long, ByVal 评论编号 As Long) As 类_SS包生成器
        Dim 读取器 As 类_读取器_外部 = Nothing
        Try
            Dim 列添加器 As 类_列添加器
            Dim 筛选器 As 类_筛选器
        ...
        If 跨进程锁.WaitOne = True Then
            Try
                ...
                If 角色 < 群角色_常量集合.成员_不可发言 Then
                    Return XML无权操作
                End If
                结果 = 数据库_删除回复(EnglishSSAddress, 角色, 流星语编号, 评论编号, 回复编号)
                If 结果.查询结果 <> 查询结果_常量集合.成功 Then
                    If 结果.查询结果 = 查询结果_常量集合.出错 Then
                        Return XML错误信息(结果.出错提示文本)
                    Else
    ...
    Private Function 数据库_删除回复(ByVal 英语讯宝地址 As String, ByVal 角色 As 群角色_常量集合, ByVal 流星语编号 As Long,
                              ByVal 评论编号 As Long, ByVal 回复编号 As Long) As 类_SS包生成器
        Dim 读取器 As 类_读取器_外部 = Nothing
        Try
            Dim 列添加器 As New 类_列添加器
            列添加器.添加列_用于筛选器("流星语编号", 筛选方式_常量集合.等于, 流星语编号)
    ...
    Private Function 数据库_保存小宇宙操作记录(ByVal 英语讯宝地址 As String, ByVal 操作代码 As 小宇宙操作代码_常量集合,
                                   ByVal 流星语编号 As Long, ByVal 评论编号 As Long, ByVal 回复对象编号 As Long) As 类_SS包生成器
        Try
            Dim 列添加器 As New 类_列添加器
            列添加器.添加列_用于筛选器("操作时间", 筛选方式_常量集合.小于, Date.UtcNow.AddHours(-24).Ticks)
            Dim 筛选器 As New 类_筛选器
            筛选器.添加一组筛选条件(列添加器)
            Dim 指令 As New 类_数据库指令_删除数据(副数据库, "小宇宙操作记录", 筛选器, "#操作时间")
            指令.执行()
            列添加器 = New 类_列添加器
            ...
            列添加器.添加列_用于插入数据("评论编号", 评论编号)
            列添加器.添加列_用于插入数据("回复对象编号", 回复对象编号)
            Dim 指令2 As New 类_数据库指令_插入新数据(副数据库, "小宇宙操作记录", 列添加器)
            指令2.执行()
            Return New 类_SS包生成器(查询结果_常量集合.成功)
        Catch ex As Exception
            Return New 类_SS包生成器(ex.Message)
